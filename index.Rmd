...
if (!suppressMessages(suppressWarnings(require("countrycode", quietly = T)))) {
  suppressMessages(suppressWarnings(install.packages("countrycode", repos = "http://cran.rstudio.com/", quiet = T, dependencies = T)))
}
...
if (!suppressMessages(suppressWarnings(require("purrr", quietly = T)))) {
suppressMessages(suppressWarnings(install.packages("purrr", repos = "http://cran.rstudio.com/", quiet = T, dependencies = T)))
}
...
df_countries <- function(protocol = c("443.https","110.pop3.starttls","25.smtp.starttls","143.imap.starttls","995.pop3s.tls","993.imaps.tls")){
...
countries_pop3s <- df_countries("995.pop3s.tls")
countries_imaps <- df_countries("993.imaps.tls")
...
```{r echo=FALSE}
mapCountryData( sPDF, 
                nameColumnToPlot='puntuacion_categories' , 
                mapTitle='Riesgo por pais y cantidad de hosts analizados' , 
...
mapBubbles(sPDF,
           nameZSize='Total',
           nameZColour="puntuacion_categories",
...
También es interesante ver la relación que existe entre el número de servidores escaneados y el nivel de riesgo. En este mapa la escala de colores nos sigue mostrando el nivel de riesgo miestras que ahora también podemos ver la cantidad de servidores escaneados en función del tamaño del circulo que se coloca en el centro de cada país.
...
ordenados_seguros<-dplyr::inner_join(ordenados_seguros,countrycode_data,by=c("key"="iso2c"),fill=0)
cat(ordenados_seguros$country.name,sep="\n")
```
 y aquellos que han obtenido una nota por debajo de 5.5 ordenados de mayor a menor nota:
...
ordenados_inseguros<-dplyr::inner_join(ordenados_inseguros,countrycode_data,by=c("key"="iso2c"),fill=0)
cat(ordenados_inseguros$country.name,sep="\n")
```
...
```{r echo=FALSE}
ordenados_seguros$country.name<-factor(ordenados_seguros$country.name, levels=unique(ordenados_seguros$country.name))
ggplot(data=ordenados_seguros, aes(x=country.name, y=puntuacion, fill=puntuacion)) +
...
```{r echo=FALSE}
ordenados_inseguros$country.name<-factor(ordenados_inseguros$country.name, levels=unique(ordenados_inseguros$country.name))
ggplot(data=ordenados_inseguros, aes(x=country.name, y=puntuacion, fill=puntuacion)) +
  geom_bar(colour="black", stat="identity") +
  guides(fill=FALSE) +
  theme(axis.text.x=element_text(angle = -90, hjust = 0)) +
...
mapCountryData( sPDF, nameColumnToPlot='puntuacion_categories' , catMethod='categorical' , mapTitle='Riesgo por pais segun HOSTs que implementan POP3' , colourPalette='palette', oceanCol='white', missingCountryCol='white')
mapBubbles(sPDF,nameZSize='Total',nameZColour="puntuacion_categories",colourPalette = "black2White",add = T, addColourLegend = F, addLegend = T, pch=21, symbolSize=1,catMethod='categorical')
```
...
mapCountryData( sPDF, nameColumnToPlot='puntuacion_categories' , catMethod='categorical' , mapTitle='Riesgo por pais segun HOSTs que implementan SMTP' , colourPalette='palette', oceanCol='white', missingCountryCol='white')
mapBubbles(sPDF,nameZSize='Total',nameZColour="puntuacion_categories",colourPalette = "black2White",add = T, addColourLegend = F, addLegend = T, pch=21, symbolSize=1,catMethod='categorical')
...
mapCountryData( sPDF, nameColumnToPlot='puntuacion_categories' , catMethod='categorical' , mapTitle='Riesgo por pais segun HOSTs que implementan IMAP' , colourPalette='palette', oceanCol='white', missingCountryCol='white')
mapBubbles(sPDF,nameZSize='Total',nameZColour="puntuacion_categories",colourPalette = "black2White",add = T, addColourLegend = F, addLegend = T, pch=21, symbolSize=1,catMethod='categorical')
...
sPDF <- joinCountryData2Map( countries_pop3s , joinCode = "ISO2" , nameJoinColumn = "key")
cutVector <- quantile(sPDF@data[["puntuacion"]],na.rm=TRUE)
sPDF@data[["puntuacion_categories"]] <- cut(sPDF@data[["puntuacion"]], cutVector, include.lowest=TRUE )
levels(sPDF@data[["puntuacion_categories"]]) <- c('Vhigh', 'High','Med', 'Low')
...
mapCountryData( sPDF, nameColumnToPlot='puntuacion_categories' , catMethod='categorical' , mapTitle='Riesgo por pais segun HOSTs que implementan POP3s' , colourPalette='palette', oceanCol='white', missingCountryCol='white')
mapBubbles(sPDF,nameZSize='Total',nameZColour="puntuacion_categories",colourPalette = "black2White",add = T, addColourLegend = F, addLegend = T, pch=21, symbolSize=1,catMethod='categorical')
...
sPDF <- joinCountryData2Map( countries_imaps , joinCode = "ISO2" , nameJoinColumn = "key")
cutVector <- quantile(sPDF@data[["puntuacion"]],na.rm=TRUE)
sPDF@data[["puntuacion_categories"]] <- cut(sPDF@data[["puntuacion"]], cutVector, include.lowest=TRUE )
levels(sPDF@data[["puntuacion_categories"]]) <- c('Vhigh', 'High','Med', 'Low')
...
mapCountryData( sPDF, nameColumnToPlot='puntuacion_categories' , catMethod='categorical' , mapTitle='Riesgo por pais segun HOSTs que implementan IMAPs' , colourPalette='palette', oceanCol='white', missingCountryCol='white')
mapBubbles(sPDF,nameZSize='Total',nameZColour="puntuacion_categories",colourPalette = "black2White",add = T, addColourLegend = F, addLegend = T, pch=21, symbolSize=1,catMethod='categorical')
